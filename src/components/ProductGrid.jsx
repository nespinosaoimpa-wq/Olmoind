...
const ProductCard = ({ product, onOpen }) => {
    ...
    return (
        <div
            ...
            style={{ cursor: 'pointer', position: 'relative', width: '180px', maxWidth: '45vw' }}
        >
            {/* Image */}
            <div style={{
                aspectRatio: '3/4',
                        ...
                        <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: '14px' }}>
                            ...
                            {(contact.addresses && contact.addresses.length > 0) ? (
                                contact.addresses.map((addr, idx) => (
                                    <li key={idx} style={{ fontSize: '14px', color: '#6b7280', display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6b7280" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: '2px' }}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                                            <span style={{ fontWeight: '600', color: '#1A1A1A' }}>{addr.name}</span>
                                            {addr.mapUrl ? (
                                                <a href={addr.mapUrl} target="_blank" rel="noreferrer" style={{ color: '#6b7280', textDecoration: 'none', transition: 'color 0.2s' }} onMouseEnter={e => e.target.style.color = '#1A1A1A'} onMouseLeave={e => e.target.style.color = '#6b7280'}>{addr.address}</a>
                                            ) : (
